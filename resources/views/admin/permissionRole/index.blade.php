                        ...
                        @if(!empty($not_in_permissions))
                            @foreach($not_in_permissions as $key=>$permission)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{URL::to('admin/permissions').'/'.$permission->id}}" class="btn btn-xs btn-warning margin-top-5">取消</a>
                                        <form action="{{ URL('admin/permissions/'.$permission->id)}}" method="POST" style="display: inline;">
                                            <input name="_method" type="hidden" value="DELETE">
                                            <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                            <button type="submit" class="btn btn-xs btn-danger margin-top-5">删除</button>
                                        </form>
                                    </td>
                                </tr>
                            @endforeach
                        ...
                        @if(!empty($in_permissions))
                            @foreach($in_permissions as $key=>$permission)
                                <tr>
                                    ...
                                    {{--<td class="">{{$permission->updated_at}}</td>--}}
                                    <td>
                                        <a href="{{URL::to('admin/permissions').'/'.$permission->id}}" class="btn btn-xs btn-info margin-top-5">授权</a>
                                        <form action="{{ URL('admin/permissions/'.$permission->id)}}" method="POST" style="display: inline;">
                                            <input name="_method" type="hidden" value="DELETE">
                                            <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                            <button type="submit" class="btn btn-xs btn-danger margin-top-5">删除</button>
